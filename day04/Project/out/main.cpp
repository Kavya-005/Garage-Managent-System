...
int parking_callback1(void *para, int f_num, char **f_value, char **f_name)
{
    ...
    {
        cout << "The field is" << f_name[0] << endl;
...
void do_update(string plate)
{
  ...
  //  cout << sql << endl;
   ...
   // cout << "333333333333333333333" << endl;
    ...
    cout << "Stayed in the garage for" << seconds << "seconds,应收费" << seconds * 0.1 << "块" << endl;
    sleep(6);
}
...
int car_in_out(int command, string &plate)
{
    ...
    cout << "Detected content:" << result.toStyledString() << endl;

    if (!result["error_code"].isNull())
    {
        cout << "error_code:" << result["error_code"] << endl;
        cout << "License plate not recognized!" << endl;
        cout << "The buzzer sounds for 1 second." << endl;
        tone(366);
        sleep(1);
        tone(0);
        sleep(1);
...
void key_event()
{
    cout << "Please press the button to release!" << endl;
    ...
    if (ev.code == KEY_UP)
    {
        ...
        if (ev.value == 1)
        {
            cout << "Pressvolum+" << endl;
        }
        light();
        cout << "Raisedvolum+" << endl;
    }
    // volum-
    if (ev.code == KEY_DOWN)
    {
    ...
    cout << "Vehicle has been released!" << endl;
    close(fd);
}
int main()
{
    ...
    string plate;
    printf("Garage Management System is starting…\n");
    while (1)
    {
        int command = -1;
        int flag = -1;
        printf("======================\n");
        printf("Entry: 1, Exit: 0, Leave Management System:2\n");
        printf("======================\n");
        scanf("%d", &command);
        if (command == 1)
        {
        ...
        {
            cout << "Pay attention to the timing of your exit." << endl;
            flag = car_in_out(command, plate);
            if (flag == 0)
                continue;
            do_update(plate);
